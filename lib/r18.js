...
module.exports = {
    ...
    detail: function(html) {
        ...
        var pubDate = $('div.product-details dd').first().text()//发行日期
        var duration = $('div.product-details dd[itemprop="duration"]').text();//时长
        var director = $('div.product-details dd[itemprop="director"]').text();//导演
        var company = $('div.product-details dd[itemprop="productionCompany"] a').text();//发行商
        var secDetailWrapper = $('div.product-details dl').last();
        var sn = $(secDetailWrapper).children('dd').eq(2).text();//番号
        var series = $(secDetailWrapper).children('dd').eq(3).text();//系列
        //抓取预览图
        var previewList = [];
